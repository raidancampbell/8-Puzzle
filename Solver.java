...
public class Solver {
    public static void main(String[] args) {
        ...
        if (response == 1) {//user's giving me the number of random moves to make
            while (!isValidInput) {
                try {
            ...
            while (!isValidInput) {
                try {
                    ...
                    for (int i = 0; i < initialBoard.length; i++) {//ensures that user's entry was numeric, and in the proper range of possible values
                        ...
                        for (int j = 0; j < initialBoard.length; j++) {//ensures no duplicates are given
                            if (Integer.parseInt(initialBoard[i]) == Integer.parseInt(initialBoard[j]) && i != j)
                                throw new IndexOutOfBoundsException();
                        }
                    }
                    if (tiles == initialBoard.length - 1)
                        isValidInput = true;//ensures that the proper number of values was given
                } catch (Exception e) {
                    System.out.println("Invalid input. Please try again.\n");
                }
            }
    ...
    public static State createPuzzle(int tiles, int response, int moves, String[] initialBoard) {//tiles and response are always given. randomMoves and initialBoard only exist based on what 'response' is
        ...
        if (tiles < 3 || initialBoard.length != tiles) {
            ...
            if (response == 2)
                return createPuzzle(initialBoard.length, 1, 0, initialBoard);//bad pass. Return the answer board
        }
        if (response != 1 && response != 2)
            return createPuzzle(8, 1, 0, new String[]{"0", "1", "2", "3", "4", "5", "6", "7", "8"});
        if (response == 2) {
            for (int i = 0; i < initialBoard.length; i++) {//ensures that user's entry was numeric, and in the proper range of possible values
        ...
        if (s.length * s.length != initialBoard.length && response == 2)
            return createPuzzle(8, 1, 0, new String[]{"0", "1", "2", "3", "4", "5", "6", "7", "8"});
